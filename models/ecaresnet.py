...
class eca_layer(nn.Module):
    ...
    def forward(self, x):
        ...
        y = self.conv(y.squeeze(-1).squeeze(-1).transpose(-1, -2)).transpose(-1, -2).unsqueeze(-1)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, k_size=[3, 3, 3, 3]):
        ...
        self.conv1 = nn.Conv3d(4, 64, kernel_size=2, stride=1, padding=1,
                               bias=False)
...
def eca_resnet10(k_size=[3, 3, 3, 3], num_classes=1_000, pretrained=False):
    ...
    """
    model = ResNet(ECABasicBlock, [1, 1, 1, 1], num_classes=num_classes, k_size=k_size)
